...
function Jl(t2) {
  ...
  return $a(_h(e), t2._key).then((n) => lf(e, t2, n));
}
class Yl extends $l {
  constructor(t2) {
...
function cf(t2, ...e) {
  ...
  return function(t3, e2, n2, s2) {
    const i2 = new ga(s2), r2 = new Ac(e2, i2, n2);
    return t3.asyncQueue.enqueueAndForget(async () => yc(await xa(t3), r2)), () => {
      i2.yc(), t3.asyncQueue.enqueueAndForget(async () => pc(await xa(t3), r2));
    };
  }(_h(a), h, u, c);
}
function hf(t2, e) {
  ...
  const template = `
    ...
    <section class='main-section'>
      ...
      <section class='dev-poly'>
        ...
        <p class='text-dev'>Me chamo Polyana Feitoza,<br>tenho 22 anos e sou do Paran\xE1.<br>Sempre foi apaixonada por doces<br> e o meu favorito \xE9 donuts.</p> 
...
const register = () => {
  ...
  const newAccountData = () => {
    ...
    createNewAccount(email, password).then(async () => {
...
const deletePost = async (postId) => {
  await of(rh(db$1, "Post", postId));
};
const updatePost = async (postId, newText, date) => {
  const postRef = rh(db$1, "Post", postId);
  await rf(postRef, {
    text: newText,
    date
  });
...
const loadPosts = (loadTimeline, uidUser) => {
  const q2 = dl(sh(db, "Post"), Il("date", "desc"));
  cf(q2, (querySnapshot) => {
    const arrayPosts = [];
